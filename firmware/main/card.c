...
void card_confirm(card_t *c, day_log_t *log, time_t now, const card_host_t *host) {
    // One tap closes the card. Checked rows are logged; unchecked ones are
    // snoozed like Delay-all does, because a card that stays up with the
    // rows you did not tick reads as the button not having worked.
    for (int i = 0; i < c->len; i++) {
        if (c->rows[i].checked) host->log_done(c->rows[i].action, host->ctx);
        else                    log->snoozed_until[c->rows[i].action] = now + CARD_SNOOZE_S;
    }
    clear(c);
...
void card_delay_all(card_t *c, day_log_t *log, time_t now, const card_host_t *host) {
    ...
    for (int i = 0; i < c->len; i++) log->snoozed_until[c->rows[i].action] = now + CARD_SNOOZE_S;
    clear(c);
    host->show_grid(host->ctx);
}
